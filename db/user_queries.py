...
async def stop_chat_timer(user_id: int):
    """
    Вычисляем, сколько минут длился чат,
    добавляем в total_minutes,
    сбрасываем chat_started_at.
    """
    try:
        user = await get_user(user_id)
        ...
        if not started:
            logger.debug("stop_chat_timer: no start time for user %s", user_id)
            return
        ...
        logger.debug("stop_chat_timer %s → +%s minutes", user_id, diff_minutes)
